...
class MessageHistoryModelTest(TestCase):
    ...
    def test_message_history_str_representation(self):
        ...
        expected_str = (
            f"Message #{self.message.pk} - Version 1 (edited by {self.editor.username})"
        )
...
class MessageEditingTest(TestCase):
    ...
    def test_pre_save_signal_no_history_for_new_messages(self):
        ...
        message = Message.objects.create(
            sender=self.sender, receiver=self.receiver, content="New message content"
        )
...
class MessageEditingIntegrationTest(TestCase):
    ...
    def test_complete_message_editing_workflow(self):
        ...
        self.assertEqual(
            first_history.old_content, "Hello, this is the original message!"
        )
        ...
        self.assertEqual(
            second_history.old_content, "Hello, this message has been edited once!"
        )
    ...
    def test_edited_at_timestamp_accuracy(self, mock_now):
        ...
        mock_time = django_timezone.datetime(
            2023, 1, 15, 12, 30, 45, tzinfo=django_timezone.utc
        )
...
class SignalUtilityFunctionTest(TestCase):
    ...
    def test_mark_all_notifications_read(self):
        ...
        message = Message.objects.create(
            sender=self.sender, receiver=self.user, content="Test message 1"
        )
        create_system_notification(
            user=self.user, title="System Alert", content="System notification"
        )
    ...
    def test_get_unread_notification_count(self):
        ...
        message2 = Message.objects.create(
            sender=self.sender, receiver=self.user, content="Test message 2"
        )
...
class IntegrationTest(TestCase):
    ...
    def test_complete_messaging_workflow(self):
        ...
        message1 = Message.objects.create(
            sender=self.user1, receiver=self.user2, content="Hello User2, how are you?"
        )
        ...
        message2 = Message.objects.create(
            sender=self.user2, receiver=self.user1, content="Hi User1, I'm doing great!"
        )
        ...
        Message.objects.create(
            sender=self.user3, receiver=self.user1, content="Hello from User3 to User1"
        )
        Message.objects.create(
            sender=self.user3, receiver=self.user2, content="Hello from User3 to User2"
        )
...
class UserDeletionSignalTest(TestCase):
    ...
    def setUp(self):
        ...
        self.message1 = Message.objects.create(
            sender=self.user1, receiver=self.user2, content="Test message 1"
        )
        self.message2 = Message.objects.create(
            sender=self.user2, receiver=self.user1, content="Test message 2"
        )
    ...
    def test_cleanup_user_data_signal_called(self, mock_print):
        ...
        mock_print.assert_any_call(
            f"Cleaning up data for deleted user: {username} (ID: {user_id})"
        )
        mock_print.assert_any_call(
            f"Successfully completed cleanup for user: {username}"
        )
...
class UserDeletionViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.message = Message.objects.create(
            sender=self.user, receiver=self.other_user, content="Test message"
        )
    ...
    def test_delete_user_view_post_without_confirmation(self):
        ...
        response = self.client.post(
            "/messaging/delete-account/", {"password": "testpass123"}
        )
    ...
    def test_delete_user_view_post_with_confirmation(self):
        ...
        response = self.client.post(
            ...
            {"confirm_deletion": "true", "password": "testpass123"},
        )
    ...
    def test_delete_user_view_ajax_request(self):
        ...
        response = self.client.post(
            "/messaging/delete-account/",
            json.dumps({"confirm_deletion": True, "password": "testpass123"}),
...
class UserDeletionIntegrationTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        for i in range(3):
            msg = Message.objects.create(
                sender=self.user1, receiver=self.user2, content=f"Message {i+1}"
            )
    ...
    def test_complete_user_deletion_workflow(self):
        ...
        # Perform deletion
        response = self.client.post(
            "/messaging/delete-account/", {"confirm_deletion": "true"}
        )
        ...
        self.assertEqual(
            MessageHistory.objects.filter(edited_by_id=self.user1.id).count(), 0
        )
    ...
    def test_foreign_key_constraint_handling(self):
        ...
        self.assertEqual(
            MessageHistory.objects.filter(edited_by_id=user1_id).count(), 0
        )
