...
class MessageAdmin(admin.ModelAdmin):
    ...
    list_filter = ["is_read", "edited", "timestamp", "edited_at", "sender", "receiver"]
    ...
    def edit_history_display(self, obj):
        ...
        for entry in history_entries:
            content_preview = (
                entry.old_content[:100] + "..."
                if len(entry.old_content) > 100
                else entry.old_content
            )
            html_parts.append(
                f"<tr>"
...
class NotificationAdmin(admin.ModelAdmin):
    ...
    search_fields = ["user__username", "title", "content", "message__content"]
    ...
    fieldsets = (
        ...
        (
            "Related Message",
            {"fields": ("message", "message_details"), "classes": ("collapse",)},
        ),
        (
            "Status & Metadata",
            {"fields": ("is_read", "created_at"), "classes": ("collapse",)},
        ),
    )
    ...
    def message_details(self, obj):
        ...
        if obj.message:
            return format_html(
                ...
                obj.message.content[:100]
                + ("..." if len(obj.message.content) > 100 else ""),
...
class MessageHistoryAdmin(admin.ModelAdmin):
    ...
    list_filter = ["edited_at", "version", "edited_by", "message__sender"]
    search_fields = [
        "message__content",
        "old_content",
        "edited_by__username",
    ...
    fieldsets = (
        ("History Entry Details", {"fields": ("message", "version", "old_content")}),
        ("Edit Information", {"fields": ("edited_by", "edited_at")}),
        (
            "Related Message Info",
            {"fields": ("message_details",), "classes": ("collapse",)},
        ),
    )
